...
2026-09-16。本文描述 Test V1.2 登录修复后的实现；成员只需持有已登记公钥对应的本地私钥。A/B/C 地址内置，不要求填写 URL、端口或访问密码。
...
自 `1.2.0-test.5` 起，本次明确输入和按服务器保存的已确认用户名优先于旧 SSH 配置；兼容旧版本保存的用户名。连接详情及失败信息显示实际尝试用户名和来源。配置未提供可靠用户名时先询问，不采用隐式的本机 OS 用户名。

已访问宿主列出的其他登记公钥可保存为本机工作区发现提示。使用备用钥匙时，在首选用户名尝试之后，可以继续验证该已知研究入口；提示不能替代 SSH 和名册认证，也不会向旧服务器追加公钥。新电脑尚无发现记录时，仍需确认首次可用入口。已有正式用户名的优先级不因备用钥匙提示而降低。
