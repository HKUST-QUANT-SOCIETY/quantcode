...
- `pytest_gate` calls the hash-pinned isolation wrapper for code/config changes and blocks when the 589-test suite fails. It may skip a docs-only PR.
...
The workflow listens for `edited` only to handle base-branch retargeting; title/body edits do not rerun 589 tests or start six additional DeepSeek reviewer pipelines.
...
The existing `Risk Gate` remains a product-domain workflow. It is not a substitute for repository-wide multi-agent code review.
