...
1. F-01/F-05：真实 SSH agent/keychain、gateway、roster 与桌面接通。八组已实现；人员表 46 条提交、39 个身份标识，37 条组员绑定和 1 条本机 Lead/Admin 运维身份已激活；张佳音按确认登记为 `model` 主组 + `factor`（Mining）授权，Session 每次只锁定一个组；指纹-only、共用公钥和未确认项仍待处理。角色和真实工作目录不能根据姓名推断。
