...
**台账尚未全通过。** 正式 roster 已激活 31 条无冲突绑定；仍缺正式 SSH gateway/MCP 会话、GitHub subject 凭据与组件服务联调。量化组件当前按“成员本地 checkout + Agent 预学习”运行，真实 API 等组件上线后再接入。当前仓库宿主已可通过独立端口加载，旧工作区的 `4096/4444` 不用于新仓库验收。前端 `frontend/packages/app/AGENTS.md` 明令禁止代理重启应用/服务器，本轮未重启用户已存在的服务。以下逐项“待验收”还包括自动回归未覆盖的真实服务、长任务与跨进程故障场景，不因汇总数字通过而自动销项。
