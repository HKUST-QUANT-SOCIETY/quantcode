...
**台账尚未全通过。** 正式 roster 已激活 37 条组员绑定和 1 条本机 Lead/Admin 运维身份；指纹-only、共用公钥和未确认身份仍拒绝登录。Server C Ubuntu systemd gateway 和本机 SSH 隧道已配置，当前只运行身份服务；GitHub token 映射仅在本机 host 配置，Server C 后台同步仍关闭，Dream consumer 也尚未交给正式进程托管。生产部署服务、组件 API 和跨机器进程托管仍待外部环境。量化组件当前按“成员本地 checkout + Agent 预学习”运行，真实 API 等组件上线后再接入。当前仓库宿主已可通过独立端口加载，旧工作区的 `4096/4444` 不用于新仓库验收。前端 `frontend/packages/app/AGENTS.md` 明令禁止代理重启应用/服务器，本轮未重启用户已存在的服务。以下逐项“待验收”还包括自动回归未覆盖的真实服务、长任务与跨进程故障场景，不因汇总数字通过而自动销项。
...
1. F-01/F-05：真实 SSH agent/keychain、gateway、roster 与桌面接通。八组已实现；人员表 46 条提交、39 个身份标识，37 条组员绑定和 1 条本机 Lead/Admin 运维身份已激活；张佳音按确认登记为 `model` 主组 + `factor`（Mining）授权，杨欣琳登记为 `model` 主组 + `agent` 授权，Session 每次只锁定一个组；当前仅剩张博睿、李卓的完整公钥缺失，以及叶易涵共用公钥/邮箱归并待处理。角色和真实工作目录不能根据姓名推断。
...
5. F-09/P-08/P-09：组织历史、个人 Pop read/ack、实时仓库权限重验、可见分支/HEAD/近期 DAG、含子目录的依赖版本及 Admin 暂存/取消已补入；Server C gateway 已注册身份服务，但 GitHub 后台同步仍需在服务器配置 token broker 后启用，Dream/Distill 仍需正式 timer/service 托管。本机系统提醒已接入待验收；通知长列表分页与全量授权未读计数已补入；静态组引用和 requirements 本仓库引用已补入；动态依赖仅可追踪声明，不执行构建代码；生产部署执行需要真实服务契约。
