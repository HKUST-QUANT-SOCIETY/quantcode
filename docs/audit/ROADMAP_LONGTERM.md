...
> 现状基线：后端 v5 回归 `1025 passed, 4 skipped`；AgentRunner、MCP 身份/组锁定、Memory/能力目录、Blackboard、Evidence、Risk CI、Admin staging、GitGraph/Pop 本地契约和六组适配器均有代码证据。桌面身份/Admin UI、GitHub 后台同步、canonical 组件连接、生产部署队列和模拟盘/实盘接口仍是外部或后续接入项。
...
| 维度 | 现状评级 | 主要缺口 |
|---|---|---|
| 多会话/并行 agent | 部分 | bounded spawn/kill/预算隔离已有，scheduler 与任务树 UI 待补 |
| 终端/代码执行+review | 部分 | 幽灵 tool 清理、PR diff 只读 tool |
| MCP 生态 | 有 | SchemaTask 接线（最小） |
| 权限与沙箱 | 部分 | permission_engine、SSH roster 后端已有，三层沙箱待接 |
| 记忆与回放 | 有 | FTS5、checkpoint/replay 已有，dream/RLHF 消费端仍为增强项 |
| 可观测性 | 部分 | trace、metrics、预算和 evidence 已有，告警/成本核算待补 |
| **行情数据接入** | **部分** | tools/market 与数据契约已有，真实 qs-data/DataAccess 连接待接 |
| **PIT 因子面板** | 部分 | FactorPanel/TargetReturnView 契约和本地 adapter 已有，生产数据源待接 |
| **回测引擎** | **部分** | 组内 strategy/options 适配器可回归，canonical A 股引擎待接 |
| **组合构建** | 部分 | Portfolio 层 construct/rebalance/verdict 已有，领域组件连接待接 |
| 实盘/模拟盘接口 | 无 | execution 组（模拟盘先行，permission 挂钩） |

**核心判断**：平台骨架（AgentRunner+MCP+Blackboard+HumanGate）与主要契约已经落地；真正的差距在**外部 canonical 数据/评估/回测/部署服务接入、桌面管理面和执行环境**。本计划优先补齐这些边界，不把组内业务再复制进 QuantCode。
...
| 算法 R6 | A1 因子挖掘工具化（submit_candidate→canonical QuantEvaluator→入池治理）；断连返回 UNAVAILABLE |
| 产品 R5 | U1 会话页三栏化 + 指标卡组件族（BigNumber/ProgressGauge/Checklist）+ **F-05 SSH 登录界面（用户点名，完整登录流程非只读卡片）** |
| **蒸馏 R7** | **P-07 CapabilityCard v2**：14 张卡（12 个 canonical 主链卡 + 公共契约/部署脚手架卡）；maturity/integration 双状态、ACL 摘要/详情、legacy 映射和复用纪律。 |
| **工作流 R10** | **P-10 Solution-First**：四维任务分类；L0/L1 直接执行，L2/L3 使用 SolutionDoc 与 conformance verdict。 |
...
| 风控 | G1-L2 模拟盘持续监控 + G2-A2 产物不可变指纹★ + G4-B1 SSH 全操作认证；QuantCode Session 的生产 SSH 写始终拒绝，部署走 Admin 管理面。 |
...
| 域 | 里程碑 |
|---|---|
| 数据 | D4 组合组件适配（TargetPortfolio/RiskBudget 契约、construct_portfolio、rebalance_plan、portfolio_verdict；越限返回 fail，不创建 HumanGate） |
...
1. **执行形态收敛为两态**：AgentRunner（ReAct 动态，产品主路径）+ compose_executor（仅 CI/兼容确定性流水线）。Model→Risk 固定链位于 `runner/risk_ci.py`。
...
4. **权限三态引擎**（ask/deny/allow）只为资源访问策略服务；普通 HumanGate 仅 merge/permission，Admin Deploy 独立。
...
7. **HumanGate 白名单（v5）**：仅 merge 与 permission。Risk/Portfolio/CI/Budget/Loop/Deploy 均使用各自状态或管理面。
...
**最高优先级单点**：canonical DataAccess/QuantEvaluator 与 qs-data 的生产连接——R3/R4/R6 三域共同依赖；本地契约和 adapter 已就绪，未接通时必须保持 `UNAVAILABLE`/`PARTIAL`，不能伪造真实评估。
