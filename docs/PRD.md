...
2. **最大复用原则**——Agent 方案首选已登记能力；已有能力覆盖不全时先向人征询，不许直接跳自造方案。蒸馏硬纪律：读生产数据必经 data_access、因子计算必走 factor_engine DSL、模型切分必复用 modeling、评估必调 quant_evaluator（禁自算 IC）。
...
| 支柱 | 编号 | 一句话 |
|---|---|---|
| 组织资产蒸馏管线 | P-07 | repo/数据 → 能力卡片（含 status 标注与 legacy 负面清单）→ 权限过滤（四档：主链全员/专项按组/纪律层/禁用清单）→ Memory 常驻目录 + FTS 细节；复用纪律（覆盖不全先问人） |
