"""Retained review configuration contracts from the original acceptance suite."""
...
REQUIRED_PROFILE_FIELDS = {
    "description", "repo_name_hints", "execution_zone", "runner_labels",
    "allowed_write_roots", "categories", "protected_outputs",
    "artifact_manifest_required_fields", "smoke_commands",
}
...
def test_reviewer_matrix_references_registered_gates() -> None:
    ...
    for reviewer_id, reviewer in reviewers.items():
        ...
        for binding in reviewer.get("programmatic_gate_bindings", []):
            ...
            assert set(binding.get("on_status", ["block", "error", "skipped"])) <= ALLOWED_STATUSES, reviewer_id


def test_quantcode_profile_is_complete_and_selectable() -> None:
    profiles = _load("repo_profiles.yaml").get("profiles")
    assert isinstance(profiles, dict) and "quantcode" in profiles
...
def test_artifact_manifest_fields_match_policy() -> None:
    policy_fields = set(_load("gate_policy.yaml")["artifact_contract"]["manifest_required_fields"])
    profile_fields = set(_load("repo_profiles.yaml")["profiles"]["quantcode"]["artifact_manifest_required_fields"])
    assert policy_fields <= profile_fields
