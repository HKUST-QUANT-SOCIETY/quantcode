...
gh workflow run quantcode-desktop.yml -R HKUST-QUANT-SOCIETY/quantcode \
  --ref main -f version=1.2.0-test.3 -f internal_test=true -f publish=true -f sign=false
```

The published title is **QuantCode Test V1.2 (1.2.0-test.3)** and the tag remains
`quantcode-v1.2.0-test.3`. The manifest reports `releaseClass=internal-test` and
