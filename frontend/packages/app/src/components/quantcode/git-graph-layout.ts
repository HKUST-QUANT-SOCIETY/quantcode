...
export function layoutGitGraph(commits: GitCommit[], heads: GitHead[] = [], main?: string) {
  // Read reactive fields once. Sorting and joins otherwise register millions
  // of duplicate Solid dependencies on a large, multi-branch repository.
  const nodes = new Map(commits.map(node => {
    const copy = { sha: node.sha, message: node.message, author: node.author, date: node.date, parents: [...node.parents] }
    return [copy.sha, copy]
  }))
  const refs = heads.map(head => ({ branch: head.branch, sha: head.sha, changed: head.changed }))
  const headsByCommit = new Map<string, GitHead[]>()
  for (const head of refs) {
    const list = headsByCommit.get(head.sha)
    if (list) list.push(head)
    else headsByCommit.set(head.sha, [head])
  }
  const dates = new Map([...nodes.values()].map(node => [node.sha, Date.parse(node.date ?? "") || 0]))
  ...
  const mainHead = refs.find(head => head.branch === main)?.sha
  const order = (a: GitCommit, b: GitCommit) =>
    (Number(b.sha === mainHead) - Number(a.sha === mainHead)) ||
    (dates.get(b.sha)! - dates.get(a.sha)!) || a.sha.localeCompare(b.sha)
  ...
  while (ready.length) {
    ...
    for (const parent of new Set(node.parents)) {
      ...
      if (children.get(parent) === 0) {
        const next = nodes.get(parent)!
        let low = 0, high = ready.length
        while (low < high) {
          const middle = (low + high) >>> 1
          if (order(ready[middle], next) <= 0) low = middle + 1
          else high = middle
        }
        ready.splice(low, 0, next)
      }
    }
  }
  ...
  const rows = sorted.map((node, index) => {
    ...
    return { ...node, row: index, lane, heads: headsByCommit.get(node.sha) ?? [] }
  })
