...
def permission_interrupt_payload(
    ...
    payload = build_interrupt_payload(
        ...
        kind="permission",
        resource=tool_id,
        actor=str(c.get("actor_id") or "") or None,
        evidence={"tool_id": tool_id, "group": group},
        reasons=[reason],
...
def enforce(tool_id: str, group: str = "", ctx: dict | None = None) -> dict[str, str]:
    ...
    decision = normalize_external_decision(raw or "")
...
# ---------------------------------------------------------------------------
# SSH actions: research reads/dev writes are allowed; production shell writes
# are denied. Admin deployment uses the separate management-plane adapter.
# ---------------------------------------------------------------------------
...
def check_ssh(action: str, target_env: str, ctx: dict | None = None) -> dict[str, str]:
    ...
    - prod_write → deny；普通研究会话和 Admin 研究会话都不是生产 shell。
    """
    from runner.server_ssh import classify_ssh_action

    cls = classify_ssh_action(action, target_env)
    if cls == "prod_write":
        raise PermissionError("production SSH writes are not available to QuantCode sessions")
...
def enforce_ssh(action: str, target_env: str, ctx: dict | None = None) -> dict[str, str]:
    """Enforce the SSH research/development boundary."""
    return check_ssh(action, target_env, ctx)
