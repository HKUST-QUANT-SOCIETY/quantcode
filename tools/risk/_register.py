...
calc_risk_tool = ToolDef(
    ...
    description=(
        ...
        "Use scenario=normal for baseline or high_risk test data."
    ),
    schema=CalcRiskArgs,
...
generate_risk_profile_tool = ToolDef(
    ...
    description=(
        ...
        "Call after calc_risk and before risk_verdict."
    ),
    schema=GenerateRiskProfileArgs,
    execute=_generate_risk_profile_execute,
)

risk_verdict_tool = ToolDef(
    id="risk_verdict",
    description=(
        "Evaluate RiskProfile against RiskThresholds. Returns verdict, breached and reasons. "
        "This is a domain result and never creates a HumanGate."
    ),
    schema=RiskVerdictArgs,
    execute=_risk_verdict_execute,
)
...
write_pr_comment_tool = ToolDef(
    ...
    description=(
        ...
        "Call after risk_verdict; CI comments report pass/fail without a HumanGate. "
...
register_tool(risk_verdict_tool)
