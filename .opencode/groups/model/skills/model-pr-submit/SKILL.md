...
4. **写入 Blackboard**：调用 `write_blackboard(key="model.pr_<pr_number>_spec", value=spec)` 写入 Blackboard
5. **触发风控**：调用 `trigger_risk_flow(blackboard_key="model.pr_<pr_number>_spec")` 通知 risk 组
